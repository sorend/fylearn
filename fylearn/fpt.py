...
def default_fuzzifier(idx, F):
    ...
    # blarg
    return [
        Leaf(idx, "low", fl.TriangularSet(v_min - (v_max - v_min) ** 2, v_min, v_max)),
...
class TreeEvaluator:
    ...
    def predict(self, node):
        ...
        if isinstance(node, Leaf):
            ...
            branches = node.branches_
            # Evaluate first branch to get size and initialize R
            v0 = self.predict(branches[0])
            R = np.zeros((v0.shape[0], len(branches)))
            R[:, 0] = v0
            for i in range(1, len(branches)):
                R[:, i] = self.predict(branches[i])
            val = node.aggregation_(R)
        else:
            val = node(self.X)
...
class FuzzyPatternTreeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def select_slaves(self, candidates, P_U_S, class_vector, X):
        ...
        for candidate in candidates:
            ...
            R.extend(_select_candidates(aggregates, self.num_slaves, class_vector, self.similarity_measure, evaluator))
    ...
    def predict(self, X):
        ...
        # However, we can use the evaluator for the predictions as well
        evaluator = TreeEvaluator(X)
        y_classes = np.zeros((X.shape[0], len(self.classes_)))
        for i, c in enumerate(self.classes_):
            y_classes[:, i] = evaluator.predict(self.trees_[i])
...
class FuzzyPatternTreeTopDownClassifier(FuzzyPatternTreeClassifier):
    ...
    def select_slaves(self, candidates, P_U_S, class_vector, X):
        ...
        for candidate in candidates:
            ...
            for c_leaf in candidate_leaves:
                for p_leaf in [p for p in P_U_S if p not in candidate_leaves]:
                    modified.extend(
                        _tree_clone_replace_leaf(c, c_leaf, Inner(aggr, [c_leaf, p_leaf])) for aggr in OPERATORS
                    )

            R.extend(
                _select_candidates(modified, self.num_candidates, class_vector, self.similarity_measure, evaluator)
            )
    ...
    def build_for_class(self, X, y, class_vector, P):
        ...
        while True:
            ...
            new_candidates = self.select_slaves(C, P, class_vector, X)
