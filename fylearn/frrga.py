...
from fylearn.ga import GenericAlgorithm
...
class FuzzyReductionRuleGAClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for class_idx, class_value in enumerate(self.classes_):
            protos[class_idx] = build_memberships(X[y == class_idx], self.membership_factory)
    ...
    def predict(self, X):
        return self.predict(self.protos_, X)
    
    def predict(self, protos=None, X):
        ...
        def predict_one(x):
            R = np.zeros(len(protos))
            for class_idx, proto in protos.items():
