...
def build_memberships(X, factory):
    ...
    return [
        (i, factory(means[i] - ((maxs[i] - mins[i]) / 2.0), means[i], means[i] + ((maxs[i] - mins[i]) / 2.0)))
...
class RandomAgreementFuzzyPatternClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for class_idx, class_value in enumerate(self.classes_):
            ...
            self.protos_[class_idx] = build_for_class_multi(
                X_class, self.max_samples, self.n_features, rs, self.membership_factory, self.n_protos
            )
